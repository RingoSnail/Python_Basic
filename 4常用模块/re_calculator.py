...
def remove_duplicates(formula):  # 计算重复运算符
...
def compute_multiply_and_dividend(formula):    # 算乘除,传进来的是字符串噢
    operators = re.findall("[*/]", formula)
    calc_list = re.split("[*/]", formula)
    res = None
    for index, i in enumerate(calc_list):
        if res:
            if operators[index - 1] == "*":  # 数字前有运算符时，运算
                res *= float(i)
            elif operators[index - 1] == "/":
                res /= float(i)
        else:
            res = float(i)  # 数字前无运算符时，直接赋值
...
def compute(formula):
    ...
    plus_and_minus_operators, multiply_and_dividend = handle_special_occasions(plus_and_minus_operators,
...
if __name__ == '__main__':
    ...
    calc("1 - 2 * ( (60-30 +(-9-2-5-2*3-5/3-40*4/2-3/5+6*3) * (-9-2-5-2*5/3 + 7 /3*99/4*2998 +10 * 568/14 )) - (-4*3)/ "
         "(16-3*2) )")
